...
const recipeSchema = yup.object({
    ...
    image : yup
        .string()
        .required('Il faut mettre une image !')
...
async function submit(values){
   try{
    ...
    if(response.ok){
        ...
        setError('generic',{type :'generic' , message : ' il ya eu une erreur'})
    }
   }catch(e){
  ...
  return (
      <form onSubmit={handleSubmit(submit)}  className="d-flex flex-column card p-20 recipeForm">
